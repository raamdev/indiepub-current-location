...
<div class="wrap">
	<div style="width:600px;">
		...
		<form name="ncl_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI'] ); ?>">
			...
			<p><?php _e( "<strong>Current Location:</strong> " ); ?>
				<input type="text" name="ncl_text" value="<?php echo stripslashes( $ncl_text ); ?>" size="20"><?php _e( " <em>(e.g. City, State, Country)</em>" ); ?>
			</p>

			<p><?php _e( "<strong>Coordinates:</strong> " ); ?>
				<input type="text" name="ncl_coords" value="<?php echo $ncl_coords; ?>" size="20"><?php _e( " <em>(e.g., 130.852855,-12.379895)</em>" ); ?>
			</p>

			<p><?php _e( "<strong>Last Updated:</strong> " ); ?> <?php echo date( DATE_RFC822, $ncl_updated_date ); ?> <?php if ( function_exists( 'time_since' ) ) { ?> (
					<em><?php echo time_since( abs( $ncl_updated_date ), time() ); ?> ago</em>) <?php } ?></p>

			<p><?php _e( "<strong>Wiki URL:</strong> " ); ?>
				<input type="text" name="ncl_wiki_url" value="<?php echo $ncl_wiki_url; ?>" size="50"><?php _e( " <br /><em>(e.g., http://en.wikipedia.org/wiki/Rapid_Creek,_Northern_Territory)</em>" ); ?>
			</p>
		...
		<form name="ncl_api_enable_form" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI'] ); ?>">
			...
			?>
			<p class="submit"><strong>Remote API is currently
					<span style="color: red;"><?php echo $ncl_api_message_text; ?></span></strong>
				<input type="submit" name="Submit" value="<?php echo $ncl_button_text; ?>" /></p>
		</form>
		<hr />
