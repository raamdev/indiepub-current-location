...
if ( isset( $_GET['ncl_api_key'] ) ) {
	...
	if ( trim( $ncl_api_key ) == "" || $ncl_api_key != $_GET['ncl_api_key'] ) {
		...
		if ( $updated == FALSE ) {
			...
			if ( $ncl_text ) {
				$updated_text = $ncl_text;
			}
			if ( $ncl_coords ) {
				$updated_text = $updated_text . " (" . $ncl_coords . ")";
			}
			if ( $ncl_wiki_url ) {
				$updated_text = $updated_text . " (" . $ncl_wiki_url . ")";
			}
			if ( $ncl_updated_date ) {
				$updated_text = $updated_text . " (" . date( DATE_RFC822, $ncl_updated_date ) . ")";
			}
